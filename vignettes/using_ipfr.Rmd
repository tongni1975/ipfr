...
Not only does the marginal distribution of persons match, but also workers and vehicles.
```{r, echo=FALSE}
wrk <- marginals %>%
    mutate(
      category = as.numeric(gsub("[A-z]", "", marginal)),
...
### Marginal agreement
So far, the marginal distributions all added up to 500 households.
...
seed$NewWeight <- ipf(seed, "count", marginals)
```

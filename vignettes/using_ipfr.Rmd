...
many disciplines to balance an initial set of weights to various marginal
...
([IPU](http://www.scag.ca.gov/Documents/PopulationSynthesizerPaper_TRB.pdf)). In
survey raking or population synthesis, the IPU algorithm has the added
...
The comparison table for the secondary (person) marginals is now availble. Feel
free to run the code chunk above for 400 or more iterations and then look again.
...
All of the following rules are checked by the functin and a warning message will
...
would make a better match, but the results are already close.

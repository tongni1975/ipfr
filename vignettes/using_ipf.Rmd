...
## Example 1: The Arizona Paper Example
...
result <- ipu(hh_seed, hh_targets, per_seed, per_targets,
              relative_gap = .0001, max_iterations = 600)
```

After running for 415 iterations, the results shown below match those found in
the paper. Running to a tighter convergence would result in an exact match, but
would take longer. The second table shows that the results match the household
type targets.
...
## Example 2: Using Multiple Geographies
...
The resulting weights match both the household and person targets.

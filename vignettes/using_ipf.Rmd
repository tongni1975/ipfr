...
table %>%
  select(-c(wrk, veh)) %>%
  group_by(siz) %>%
  summarize_each(funs(round(sum(.), 1))) %>%
  gather(key = ID, value = result, -siz) %>%
  spread(key = siz, value = result) %>%
  htmlTable(
...
table %>%
  ...
  group_by(wrk) %>%
  summarize_each(funs(round(sum(.), 1))) %>%
  gather(key = ID, value = result, -wrk) %>%
  spread(key = wrk, value = result) %>%
  htmlTable(
...
table %>%
  ...
  group_by(veh) %>%
  summarize_each(funs(round(sum(.), 1))) %>%
  gather(key = ID, value = result, -veh) %>%
  spread(key = veh, value = result) %>%
  htmlTable(

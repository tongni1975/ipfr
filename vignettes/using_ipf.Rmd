...
Run `ipu()`. Note that it picks up a new error. When providing cluster values in
the seed table, `ipfr` checks to make sure that every marginal category appears
in every cluster. In this case, the full seed table does contain both size 1 and
2 records, but cluster 2 only has size 1.
...
attributes that describe people (like age). This is possible with the `ipu()` function.
...
per_seed <- data_frame(
  hhid = c(1, 1, 1, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 7, 7, 7, 7, 7, 8, 8),
...
only run for 10 iterations. After running for 400 or more iterations, the 
results match closely to those shown in the paper.

```{r example 1 ipu}
result <- ipu(hh_seed, hh_targets, per_seed, per_targets,
              relative_gap = .0001, max_iterations = 10)
```

The first table shows a sample of the result. The second table shows how well the
results match targets when summarized. Feel free to run the code chunk above for
400 or more iterations and then look again.

```{r example 1 results}
head(result) %>%
  htmlTable(
    rnames = FALSE, caption = "Sample of Resulting Seeds"
  )
...
result %>% 
  ...
  htmlTable(
    rnames = FALSE, caption = "Summary of Resulting Weights by Geography"
  )
```
...
show if one is violated.
  ...
  * All fields that designate geographies must start with "geo_"
    ...
    * geo_region
    * geo_state
  * Do **not** put any geo fields on the person seed table.
    * This prevents any errors/inconsistencies between seed tables.
...
hh_seed <- bind_rows(
  hh_seed,
  hh_seed %>% 
    mutate(geo_cluster = 2, hhid = hhid + 8)
)
...
Run the IPU algorithm. Again, for vignette build time, only 10 iterations are
...
```{r example 2 results}
result %>% group_by(hhtype) %>% summarize(weight = sum(weight))

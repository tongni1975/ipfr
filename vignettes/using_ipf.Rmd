...
using the `min_weight_scale` and `max_weight_scale` variables.
...
* max_weight_scale = 5 (5x the average weight) * min_weight_scale = .2 (1/5 the
average weight)
...
the same seed and target data from Example 1.
...
result <- ipu(hh_seed, hh_targets, max_iterations = 10,
              max_weight_scale = 1.2, min_weight_scale = .8)
```
...
Even with reasonable values for the weight caps, the 
minimum allowable weight is much higher than 5. This is an extreme example, and
is unlikely to be an issue in applications related to housing and population -
the targets are generally on the same scale. However, when expanding a through
trip table, it is common to have some external stations with large targets and
others with small. In these cases, it is advisable to leave the scale arguments
